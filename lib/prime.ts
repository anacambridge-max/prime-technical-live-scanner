...
function nearestLevel(price: number, levels: Record<string, number>, tolerancePct = 0.005) {
...
function rowFromCandles(symbol: string, candles: Candle[], daily?: DailyQuote): ScanRow {
  ...
  if (!latest || !previous) {
    return { symbol, ltp: 0, change: 0, volumeX: 0, ema: '—', level: '—', setup: '—', status: 'NO TRADE', entry: null, sl: null, target: null, reason: 'Insufficient 5-minute candles', signalTime: null };
  }
  ...
  // PDH / PDL are the PRIMARY levels. OR levels are secondary context only.
  ...
  // Always prefer PDH/PDL in the displayed Level column.
  ...
  let level = '—';
  if (pdhDistance <= 0.005 && pdhDistance <= pdlDistance) level = 'PDH';
  else if (pdlDistance <= 0.005) level = 'PDL';
  ...
  // Only show WATCH rows when PDH/PDL is genuinely relevant (within 0.5%),
  // instead of filling the dashboard with hundreds of generic EMA watches.
  const nearPdh = Number.isFinite(pdh) && pdhDistance <= 0.005;
  const nearPdl = Number.isFinite(pdl) && pdlDistance <= 0.005;

  let status: ScanRow['status'] = 'NO TRADE';
  let setup = '—';
  let reason = 'No current PDH/PDL Prime Technical setup';
  ...
  if (pdhConfirmed) {
    ...
    status = 'WATCH';
    setup = nearPdh && nearPdl ? 'PDH/PDL WATCH' : nearPdh ? 'PDH WATCH' : 'PDL WATCH';
